...
| 优先级 | 平台 | 类型 | 用途 |
|---|---|---|---|
| 1 | RightCode | 极致低价 | Claude / GPT / Gemini 低价补充，低权重 |
| 2 | PoloAPI / weelinking | 稳定中价 | 主力稳定渠道，高权重 |
| 3 | SiliconFlow | 国产最优 | DeepSeek、Qwen、豆包、GLM、Embedding 主力 |
| 4 | jiekou.ai / APIMart / token.chhai.cn | 全能补充 | 模型补货、图像视频、备用 |

热门模型至少保留 2-3 个渠道。稳定渠道权重高，低价渠道权重低，开启失败自动切换。
...
1. 添加 SiliconFlow，先开放 DeepSeek、Qwen、豆包、GLM、Embedding。
2. 添加 PoloAPI 或 weelinking，作为 Claude / GPT / Gemini 稳定主力。
3. 添加 RightCode，作为 Claude / GPT / Gemini 的低价补充，低权重。
4. 添加 jiekou.ai、APIMart、token.chhai.cn，做模型补货和图像视频备用。
5. 每个热门模型设置至少 2-3 个渠道，开启 failover。
6. 配置分组倍率和用户等级：免费试用、普通、VIP、企业。
